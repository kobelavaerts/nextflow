...
{ref}`Workflows <workflow-page>` can now use type annotations to describe workflow inputs and outputs.

(workflow-params-typed)=
...
When a script enables the `nextflow.preview.types` feature flag, workflows in the script use *typed dataflow* semantics, which introduces the following new behaviors:
...
When typed dataflow is enabled, channels use a new set of operators.
...
See {ref}`operator-typed-page` for more information about each operator. See {ref}`migrating-typed-dataflow` to learn how to migrate existing code to use typed dataflow.
...
With typed dataflow, the same behavior can be achieved by calling the process with named arguments:
...
The following syntax patterns are no longer allowed when typed dataflow is enabled.
...
The {ref}`special operators <workflow-special-operators>` `|` and `&` cannot be used with typed dataflow. Use standard method calls instead:
...
The implicit `.out` property for process and workflow outputs cannot be used with typed dataflow. Use standard assignments instead:
