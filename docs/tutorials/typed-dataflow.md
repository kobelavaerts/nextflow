(migrating-typed-dataflow)=

# Migrating to typed dataflow

Nextflow 26.04 introduces *typed dataflow* semantics for scripts that enable the `nextflow.preview.types` feature flag. {ref}`Typed dataflow <dataflow-typed>` introduces a number of changes to workflows, including a streamlined operator library.

While some existing operators are supported by typed dataflow with only minor changes, the vast majority of existing operators are not supported. This tutorial demonstrates how to migrate each existing operator to typed dataflow.

See {ref}`operator-typed-page}` for the full list of operators that are supported with typed dataflow.

## Supported with no changes

The following operators are supported in typed dataflow with no changes.
...
The following operators are supported in typed dataflow with minor changes. These operators can usually be carried over with no changes, but may require minor modifications in certain cases.
...
The following operators are supported in typed dataflow with significant changes in behavior. Make sure to study these changes carefully if you use any of these operators.
...
The `groupTuple` operator is replaced by `groupBy` in typed dataflow.
...
A useful way to understand `groupBy` is that it is effectively the inverse of `flatMap`. For example:

```nextflow
channel.of(
        record(chr: 'chr1', vcfs: ['reg1_chr1.vcf', 'reg2_chr1.vcf']),
        record(chr: 'chr2', vcfs: ['reg1_chr2.vcf', 'reg2_chr2.vcf', 'reg3_chr2.vcf']),
    )
    .flatMap { g ->
        g.vcfs.collect { vcf ->
            tuple(g.chr, g.vcfs.size(), vcf)
        }
    }
    .groupBy()
    .view()
...
```
[key: chr1, values: [reg1_chr1.vcf, reg2_chr1.vcf]]
[key: chr2, values: [reg1_chr2.vcf, reg2_chr2.vcf, reg3_chr2.vcf]]
```
...
The `join` operator does not support channels of tuples in typed dataflow. You must refactor incoming channels to use records instead of tuples. See {ref}`migrating-records` for more information on how to migrate to records.
...
Use `view` instead. The `view` operator in typed dataflow supports the same `tag` option, allowing it to be used like `dump`.
...
Both cases can be implemented in typed dataflow without `ifEmpty`:
...
The `randomSample` operator has no equivalent in typed dataflow. In general, this operator should not be used because it leads to non-deterministic behavior.
