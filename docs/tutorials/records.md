...
At this point, you need to enable the `nextflow.preview.types` feature flag so that the workflow uses *typed dataflow* semantics. Typed dataflow introduces a new `join` operator that operators on records instead of tuples.

With typed dataflow enable, rewrite the `join` operator to join on the `id` field:
