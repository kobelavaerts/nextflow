...
Dataflow logic in Nextflow consists of composing processes, channels, and *dataflow operators* (or just *operators*). Operators are essential for transforming, filtering, and combining channels to control the flow of data through a pipeline.
...
- Discourage the use of syntax variants that do not provide enough value to the language

## Non-goals

- Remove support for existing dataflow semantics -- typed dataflow should be opt-in
...
Introduce new dataflow semantics with the `nextflow.preview.types` feature flag:
...
// goodbye.nf
workflow GOODBYE {
    // legacy dataflow semantics
}
```

Like typed processes, this flag must be specified in every script that uses typed dataflow.

**TODO:** can typed dataflow scripts still be mixed with legacy dataflow scripts ?

### New operators

Typed dataflow introduces a new set of operators for the `Channel` type. When typed dataflow is enabled, channels will use these operators instead of the legacy operators.
...
Legacy operators that are not included in typed dataflow can be migrated as follows:
...
In most cases, legacy operators can be replaced by a combination of core operators and regular functions (e.g., `Iterable::toSorted()`, `Path::splitCsv()`). The accompanying migration guide provides detailed examples for each operator.

### Fewer syntax variants

Typed dataflow disallows the following syntax variants:
...
This aspect of programming languages is becoming more salient as more code is being read and written by AI agents. Agents need many examples of a programming language in order to use it effectively, so when a language has many syntax variants or syntax that deviates heavily from the common patterns used by other languages, it hurts the agent's ability to read and write code in that language.

### Process named arguments

When a process takes inputs from multiple sources (e.g. a *sample* and an *index*), the simplest approach is to declare them as separate inputs:
...
Then the calling workflow would call the process as follows:

```groovy
workflow FASTQ_ALIGN_DEDUP {
    take:
    ch_samples: Channel<Sample>
    index: Value<Path>

    main:
    ALIGN(ch_samples, index)
...
However, this approach assumes the calling workflow will have either a fixed sample or a fixed index. Since a process cannot be called with multiple channel inputs, this approach does not support the case of multiple samples and multiple indices.

The most generic approach, then, is to combine the sample and index into a single record input:
...
But now the calling workflow needs to combine the samples and index into a single channel:
...
The extra logic is reasonable when there are actually multiple samples and multiple indices. However, in the case of a fixed index (which is much more common), the calling workflow is now much more verbose than before.

This example illustrates a trade-off that now exists with process record inputs -- making processes generic is in conflict with making them simple to call in the most common cases.

With typed dataflow, the same behavior can be achieved using named arguments:
